...
describe('Model', function() {
  ...
  describe('handlers', function() {
    it('should handle schema request', function(done) {
      request(app)
        ...
        .end(function(err, res) {
          ...
          res.body.list_uri.should.equal('/api.js/movies');
          res.body.detail_uri.should.equal('/api.js/movies/:id');
          done();
        });
    });
    ...
    it('should fail on GET missing resource', function(done) {
      request(app)
        .get('/api.js/movies/55e8169191ad293c221a6c9d')
        .expect(404, done);
    });
    it('should fail on PUT missing resource (shouldUseAtomicUpdate=false)', function(done) {
      request(app)
        .put('/api.js/genres/55e8169191ad293c221a6c9d')
        .send({
    ...
    it('should fail on PUT missing resource (shouldUseAtomicUpdate=true)', function(done) {
      request(app)
        .put('/api.js/movies/55e8169191ad293c221a6c9d')
        .send({
    ...
    it('should fail on DELETE missing resource (shouldUseAtomicUpdate=false)', function(done) {
      request(app)
        .del('/api.js/genres/55e8169191ad293c221a6c9d')
        .expect(404, done);
    });
    ...
    it("shouldn't put data on deleted resource", function(done) {
      request(app)
        .del('/api.js/movies/' + config.movies[5]._id)
        .end(function(err, res) {
          request(app)
            .put('/api.js/movies/' + config.movies[5]._id)
            .send({
    ...
    it('should 400 deleting a resource twice', function(done) {
      request(app)
        .del('/api.js/movies/' + config.movies[6]._id)
        .end(function() {
          request(app)
            .del('/api.js/movies/' + config.movies[6]._id)
            .expect(404, done);
    ...
    it('should return a nested model at the generated endpoint creator', function(done) {
      request(app)
        .get('/api.js/movies/' + movie1._id + '/creator')
        .expect('Content-Type', /json/)
        .expect(200)
        .end(function(err, res) {
    ...
    it('should 404 if we request an object endpoint without a filter', function(done) {
      request(app)
        .get('/api.js/movies/creator')
        .expect(404, done);
    });
    it('should retrieve a deeply nested endpoint', function(done) {
      request(app)
        .get('/api.js/movies/' + movie1._id + '/meta/director')
        .expect('Content-Type', /json/)
        .expect(200)
        .end(function(err, res) {
          res.should.be.json;
    ...
    it('should 404 on a nested object', function(done) {
      request(app)
        .get('/api.js/movies/' + movie1._id + '/meta')
        .expect(404, done)
    });
    it('should get a user defined route', function(done) {
      request(app)
        .get('/api.js/movies/recommend')
        .expect('Content-Type', /json/)
        .expect(200)
        .end(function(err, res) {
          res.should.be.json;
    ...
    it('should get anotheroute (user defined route)', function(done) {
      request(app)
        .get('/api.js/movies/anotherroute')
        .expect('Content-Type', /json/)
        .expect(200)
        .end(function(err, res) {
          res.should.be.json;
    ...
    it('should get athirdroute (user defined route)', function(done) {
      request(app)
        .get('/api.js/movies/' + movie1._id + '/athirdroute')
        .expect('Content-Type', /json/)
        .expect(200)
        .end(function(err, res) {
          res.should.be.json;
    ...
    it('should fail athirdroute (user defined route)', function(done) {
      request(app)
        .get('/api.js/movies/athirdroute')
        .expect(404, done);
    });
    it('should fail athirdroute (user defined route)', function(done) {
      request(app)
        .put('/api.js/movies/' + movie1._id + '/athirdroute')
        .expect(404, done);
    });
    it('should allow put of entire object', function(done) {
      request(app)
        .get('/api.js/movies/' + config.movies[7]._id)
        .end(function(err, res) {
          var movie = res.body;
          movie.title = 'A different title';
          request(app)
            .put('/api.js/movies/' + movie._id)
            .send(movie)
            .expect('Content-Type', /json/)
            .expect(201)
            .end(function(err, res) {
